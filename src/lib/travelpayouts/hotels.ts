...
export function buildHotelSearchUrl(
  ...
  return {
    hotellook: `https://search.hotellook.com/hotels?destination=${encodedDest}&checkIn=${checkIn}&checkOut=${checkOut}&adults=${adults}&marker=${marker}`,
    ...
    agoda: `https://tp.media/r?marker=${marker}&p=3838&u=${encodeURIComponent(
      `https://www.agoda.com/search?city=${destination}&checkIn=${checkIn}&checkOut=${checkOut}&rooms=1&adults=${adults}`
    )}`,
    expedia: `https://tp.media/r?marker=${marker}&p=6498&u=${encodeURIComponent(
      `https://www.expedia.com/Hotel-Search?destination=${destination}&startDate=${checkIn}&endDate=${checkOut}&adults=${adults}`
    )}`,
  };
}
